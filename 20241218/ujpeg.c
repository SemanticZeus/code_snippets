...
unsigned char * ujCreate(void) {
    ujContext *uj = (ujContext*) calloc(1, sizeof(ujContext));
    ujError = uj ? UJ_OK : UJ_OUT_OF_MEM;
    return (unsigned char *) uj;
}

void ujDisableDecoding(unsigned char * img) {
...
void ujSetChromaMode(unsigned char * img, int mode) {
    ujContext *uj = (ujContext*) img;
    if (uj) {
        uj->fast_chroma = mode;
        ujError = UJ_OK;
...
unsigned char * ujDecode(unsigned char * img, const void* jpeg, const int size) {
...
unsigned char * ujDecodeFile(unsigned char * img, const char* filename) {
    FILE *f; size_t size;
    void *buf;
    ujError = UJ_OK;
    f = fopen(filename, "rb");
...
unsigned char* ujGetImage(unsigned char * img, unsigned char* dest) {
    ujContext *uj = (ujContext*) img;
    ujError = !uj ? UJ_NO_CONTEXT : (uj->decoded ? UJ_OK : UJ_NOT_DECODED);
