...
int transmit_nrf24(Package *p, int n) {
  ...
  Serial.println("after initiating transmission");
  uint8_t waiting[31];
  memset(waiting, 1, 31);
  radio.openWritingPipe(0, address);
  radio.stopListening();
...
void loop() {
    for (int i=0;i<max_num_packages;i++) {
      packages[i].number = i;
      snprintf(packages[i].data, 31, "%d Heather Mason", counter++ % 100);
      if (transmit_nrf24(packages, max_num_packages)) Serial.println("Transmission Error Occured");
    }
}
